...
public class AwardServiceImpl implements AwardService {
    ...
    @Override
    @Transactional
    public AwardExecution modifyAward(Award award, Long shopId, ImageHolder imageHolder) {
        if (award.getPoint() != null){
            int effectNum;
            if (imageHolder.getImage() != null && imageHolder.getImageName() != null){
                ...
                effectNum = awardDao.updateAward(award, shopId);
            }else{
                //表示不需要更新奖品图片
                effectNum = awardDao.updateAward(award, shopId);
            }
