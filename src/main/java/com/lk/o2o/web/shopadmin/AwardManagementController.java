...
public class AwardManagementController {
    ...
    private Map<String,Object> modifyAward(HttpServletRequest request){
        ...
        if(currentShop != null && currentShop.getShopId() != null){
            ...
            try {
                ...
                if (commonsMultipartResolver.isMultipart(request)){
                    ...
                    if (thumbnailFile != null){
                        imageHoleder = new ImageHolder(thumbnailFile.getInputStream(),thumbnailFile.getOriginalFilename());
                    }
                }
            }catch (Exception e){
                modelMap.put("success",false);
            ...
            try {
                ...
                //传入奖品修改信息，店铺id，图片信息
                AwardExecution ae = awardService.modifyAward(award, imageHoleder);
                if (ae.getState() == AwardStateEnum.SUCCESS.getState()){
                    modelMap.put("success",true);
                }else{
                    modelMap.put("success",false);
    ...
    private Map<String,Object> addAward(HttpServletRequest request){
        ...
        if (currentShop != null && currentShop.getShopId() != null){
            ...
            try {
                ...
                award.setShopId(currentShop.getShopId());
                //传入奖品修改信息，店铺id，图片信息
                AwardExecution ae = awardService.addAward(award, imageHoleder);
                if (ae.getState() == AwardStateEnum.SUCCESS.getState()){
                    modelMap.put("success",true);
                }else{
                    modelMap.put("success",false);
    ...
    @ResponseBody
    private Map<String,Object> getAwardById(@RequestParam Long awardId , HttpServletRequest request){
        Map<String, Object> modelMap = new HashMap<>();
        //从session中取出店铺信息
        Shop currentShop =(Shop) request.getSession().getAttribute("currentShop");
        //判断空值
        if (awardId < 0 || currentShop == null || currentShop.getShopId() == null){
            modelMap.put("success",false);
            modelMap.put("errMsg","Important info empty");
            return modelMap;
        }
        Award award = awardService.queryAwardById(awardId, currentShop.getShopId());
        if (award != null){
            modelMap.put("success",true);
